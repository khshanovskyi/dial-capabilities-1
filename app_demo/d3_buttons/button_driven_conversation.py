...
class StateMachineApplication(ChatCompletion):
...
app = DIALApp()
app.add_chat_completion("buttons-sample", StateMachineApplication())

if __name__ == "__main__":
    import sys

    if 'pydevd' in sys.modules:
        config = uvicorn.Config(app, port=5027, host="0.0.0.0", log_level="info")
        server = uvicorn.Server(config)
        import asyncio

        asyncio.run(server.serve())
    else:
        uvicorn.run(app, port=5027, host="0.0.0.0", log_level="info")
